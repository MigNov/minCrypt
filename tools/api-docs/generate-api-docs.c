...
int main(int argc, char *argv[])
{
	...
	if (argc < 3) {
		fprintf(stderr, "Syntax: %s [-p|--private] [-c|--clib] source-file output-in-file\n", argv[0]);
		return 1;
	}
	...
	if ((strcmp(argv[idx], "-c") == 0) || (strcmp(argv[idx], "--clib") == 0)) {
		if (argc < idx + 2) {
			fprintf(stderr, "Syntax: %s [-p|--private] [-c|--clib] source-file output-in-file\n", argv[0]);
			return 1;
		}
	...
	fprintf(fp,"<h1>%s API Reference guide</h1>\n\n    <h3>Functions</h3>\n\n    <!-- Links -->\n",
			(private == 0) ? ((clib == 0) ? "PHP" : "C") :
			(clib == 0) ? "PHP Developer's" : "C Developer's");
